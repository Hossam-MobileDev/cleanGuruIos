...
struct CalendarCleanerView: View {
            ...
            if isLoading {
                ...
                ScrollView {
                    LazyVStack(spacing: 0) {
                        ForEach(calendarEvents, id: \.safeEventIdentifier) { event in
                            CalendarEventRow(
                                event: event,
                                isSelected: selectedEventIds.contains(event.safeEventIdentifier),
    ...
    private func fetchCalendarEvents() {
        ...
        // Filter out events with nil identifiers and sort by date
        let validEvents = events.filter { $0.eventIdentifier != nil }
    ...
    private func toggleEventSelection(event: EKEvent) {
        let eventId = event.safeEventIdentifier
    ...
    private func deleteSelectedEvents() {
        ...
        for eventId in selectedEventIds {
            // Since we're using safeEventIdentifier, we need to find the event differently
            if let eventToRemove = calendarEvents.first(where: { $0.safeEventIdentifier == eventId }) {
                do {
        ...
        do {
            ...
            // Wait a moment before refreshing to ensure changes are saved
...
// Extension to safely handle event identifiers
extension EKEvent {
    var safeEventIdentifier: String {
        return eventIdentifier ?? UUID().uuidString
    }
}

// Custom button style for row highlighting
...
// Calendar Event Row - Full row is clickable
struct CalendarEventRow: View {
    ...
    var body: some View {
        Button(action: onToggle) {
            ...
            .background(Color(UIColor.systemBackground)) // Add background to ensure full tap area
            .contentShape(Rectangle()) // This ensures the entire row area is tappable
        }
        .buttonStyle(RowButtonStyle()) // Custom style for press highlighting
    }
