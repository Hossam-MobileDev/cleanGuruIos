...
struct ContactsCleanupView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Action buttons section - Only show when permission is granted
            if !permissionDenied {
                HStack(spacing: 20) {
            ...
            if isLoading {
                ...
                VStack {
                    ...
                    VStack(spacing: 20) {
                        ...
                        Text(LocalizedStrings.string(for: "contacts_access_required", language: languageManager.currentLanguage))
                            .font(.headline)
                            .multilineTextAlignment(.center)
                        
                        Text(LocalizedStrings.string(for: "please_allow_access_analyze", language: languageManager.currentLanguage))
                            .multilineTextAlignment(.center)
                    ...
                    ScrollView {
                        if duplicateGroups.isEmpty || duplicateGroups.values.allSatisfy({ $0.count <= 1 }) {
                            ...
                            VStack(spacing: 20) {
                                ...
                                Text(LocalizedStrings.string(for: "no_duplicates_found", language: languageManager.currentLanguage))
                                    .font(.headline)
                                    .multilineTextAlignment(.center)
                                
                                Text(LocalizedStrings.string(for: "great_no_duplicates", language: languageManager.currentLanguage))
                                    .multilineTextAlignment(.center)
                                    .foregroundColor(.secondary)
                                    .padding(.horizontal)
                            }
        ...
        .alert(LocalizedStrings.string(for: "backup_successful", language: languageManager.currentLanguage), isPresented: $showBackupSuccessAlert) {
            Button(LocalizedStrings.string(for: "ok", language: languageManager.currentLanguage), role: .cancel) { }
        } message: {
            Text(LocalizedStrings.string(for: "backup_created", language: languageManager.currentLanguage))
        }
        .alert(LocalizedStrings.string(for: "restore_successful", language: languageManager.currentLanguage), isPresented: $showRestoreSuccessAlert) {
            Button(LocalizedStrings.string(for: "ok", language: languageManager.currentLanguage), role: .cancel) { }
        } message: {
